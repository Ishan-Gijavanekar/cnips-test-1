...
module.exports = {
    ...
    execute: async function(event, ctx, config) { /* required */
        ...
        /*let contactResponse = await createContact(event.data.userData,config)
        console.log("contactCreated set to: ", contactResponse);
        event.data.contactResponse = contactResponse;*/
        console.log('DESTINATION:FD_DEST_CONTACT', "Event: ", event);
        //return event;
    },
...
async function createContact(userInfo, config){
    let responseObject = {
        createContact: false,
        data: null,
        error: null
    ...
    try{
        if (config && config.company_id){
            let response = await axios.post(config.freshdesk_url +`/api/v2/contacts`,
                {
            ...
            if (!response || !response.data || response.status !== 201) {
                console.log('error creating contact. response status', response.status, " response data: ", response.data);
