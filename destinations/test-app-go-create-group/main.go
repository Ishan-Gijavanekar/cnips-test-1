...
func Execute(config map[string]string, event string, vars map[string]string, log *log.Logger) (map[string]string, error) {
	...
	resp, err := CreateGroup(incomingEvent.GroupInfo, incomingEvent.Token, baseUrl)
	if err != nil {
		log.Debug(fmt.Sprintf("error creating group: %v ", err))
		return nil, fmt.Errorf("error creating group: %v", err)
	}
	log.Debug(fmt.Sprintf("created group response: %v ", resp))
	return vars, nil
}

func CreateGroup(request GroupRequest, token, baseUrl string) (string, error) {
	requestBody, err := json.Marshal(request)
	if err != nil {
		return "", fmt.Errorf("failed to marshal groupInfo  body: %v", err)
	}
	...
	if err != nil {
		...
		return "", errors.Errorf("error making create group request %s - error %s", apiURL, err)
	...
	fmt.Println("Group Response: ", string(body))
