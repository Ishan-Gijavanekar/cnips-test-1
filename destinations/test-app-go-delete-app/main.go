...
func Execute(config map[string]string, event string, vars map[string]string, log *log.Logger) (map[string]string, error) {
	...
	appResp, err := DeleteApp(baseUrl, incomingEvent.AppId, token)
	if err != nil {
		log.Debug(fmt.Sprintf("error deleting app: %v ", err))
		return nil, fmt.Errorf("error deleting app: %v", err)
	}
	log.Debug(fmt.Sprintf("deleted app response: %v ", appResp))
	return vars, nil
}

func DeleteApp(baseURL, appId, token string) (string, error) {
