...
func Execute(config map[string]string, event string, vars map[string]string, log *log.Logger) (map[string]string, error) {
	...
	appResp, err := UpdateApp(incomingEvent.RequestAppInfo, token, baseUrl)
	if err != nil {
		log.Debug(fmt.Sprintf("error updating app: %v ", err))
		return nil, fmt.Errorf("error updating app: %v", err)
	}
	log.Debug(fmt.Sprintf("updated app response: %v ", appResp))
	return vars, nil
}

func UpdateApp(request RequestAppInfo, token, baseUrl string) (*AppInfo, error) {
