...
func (t *TransformationImpl) Execute(args shared.Args) (shared.Args, error) {
	...
	// Do some transformation with your event
	myEvent.Processed = true
	args.Log.Info("This is just a info log message")
	// marshal your event back to json and update in args
	eventJson, err := json.Marshal(myEvent)
	if err != nil {
		return args, err
	}
	args.Event = string(eventJson)
	// always return the args with your event otherwise pipeline will stop processing
	return args, nil
}
